...
if ( ! function_exists( 'goodminimal_setup' ) ):
	...
	function goodminimal_setup() {
		...
		if (!is_admin()) {
			...
			wp_enqueue_script('jquery_twitterfeed', get_template_directory_uri().'/js/twitterfeed.js', false, '1.0', false);
			// add parameters for the JavaScript
			wp_localize_script('jquery_twitterfeed', 'twitter_load_parameters', array('theme_default_path' => get_template_directory_uri()));
		}
...
//load the drop down menu for mobile devices
...
$options = array (
	...
	array(	"name" => "Consumer Key",
			"desc" => "",
			"id" => $shortname."_twitter_consumerkey",
			"type" => "text"),
			
	array(	"name" => "Consumer Secret",
			"desc" => "",
			"id" => $shortname."_twitter_consumersecret",
			"type" => "text"),
			
	array(	"name" => "Access Token",
			"desc" => "",
			"id" => $shortname."_twitter_accesstoken",
			"type" => "text"),
			
	array(	"name" => "Access Token Secret",
			"desc" => "",
			"id" => $shortname."_twitter_accesstoken_secret",
			"type" => "text"),			
