<?php
/*
Template Name: Portfolio Categorized
*/
get_header();
?>
	...
	<div class="center_wrap">
		...
		<section id="content" class="full_width container_shadow">
				...
				<ul id="filters" class="option-set clearfix" data-option-key="filter">
					...
					<?php
						...
						foreach($categories as $category){
							$cat_number++;
							echo '
								<li>/<a href="#filter" data-option-value=".'.$category->category_nicename.'"> '.$category->cat_name.' </a></li>
							';
						}
					?>
				</ul>
			...
			<div id="portfolio">
				...
				<ul class="display_thumbnail three_col" id="image-grid" style="heigth:100%;">
				<?php
					$thePostID = $post->ID;
					...
					if ($wp_query->have_posts()) : while($wp_query->have_posts()) : $wp_query->the_post();
						
						{
							...
							$portfolio_image = get_the_post_thumbnail($post->ID, 'portfolio_three', array('title' => the_title_attribute('echo=0')));
							...
							if ($i == 4) {
								$i = 1;
								echo '<li class="thumb categoryelement '.$cat_slug.'" data-category="'.$cat_slug.'" style="clear:left;">';
							} else {
								echo '<li class="thumb categoryelement '.$cat_slug.'" data-category="'.$cat_slug.'" >';
							}
			  ...
			  <script src="<?php echo get_template_directory_uri(); ?>/js/libs/jquery.isotope.min.js"></script>
				  <script>
					$(function(){
